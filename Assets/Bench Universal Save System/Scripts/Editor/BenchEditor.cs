...
namespace Terresquall {
    [CustomEditor(typeof(LightSpot), true)]
    public class BenchEditor : Editor {
        ...
        void OnEnable() {
            lightSpot = target as LightSpot;
        }

        public static LightSpot FindSettings() {
            string[] guids = AssetDatabase.FindAssets("t:ScriptableObject");
            foreach (string s in guids) {
                // Load the first found settings asset
                string path = AssetDatabase.GUIDToAssetPath(s);
                ScriptableObject data = AssetDatabase.LoadAssetAtPath<LightSpot>(path);
                if (data is LightSpot) return data as LightSpot;
            }
            return null;
        }
        ...
        public override void OnInspectorGUI() {
            ...
            if(EditorApplication.isPlaying) {
                ...
                EditorGUILayout.TextField("Current Save Slot", LightSpot.currentSlot.ToString());
                EditorGUILayout.TextField("Last Playtime", LightSpot.lastPlaytime.ToString());
                ...
                if (currentSaveFoldout) {
                    ...
                    LightSpot.SaveFile sav = LightSpot.GetCurrentSaveFile();
                    if (sav == null) EditorGUILayout.LabelField("No save data.");
                    else RenderSavePropertyString(LightSpot.GetCurrentSaveFile().ToString());
                    EditorGUI.indentLevel--;
                    EditorGUILayout.EndVertical();
                }
            ...
            for (int i = 0; i < lightSpot.maxSaveSlots; i++) {
                ...
                if (data.ContainsKey(i)) {
                    ...
                    if (LightSpot.SlotHasSave(i)) {
                        ...
                        if (GUILayout.Button($"View Saved Data in Slot {i}")) {
                            ...
                            LightSpot.SaveFile save = LightSpot.LoadGame(i);
                            if (save == null) {
            ...
            if (GUILayout.Button("Open Save Path")) OpenPath(LightSpot.GetSavePath());
        ...
        void OpenPath(string subfolder) {
            string path = LightSpot.GetSavePath();
